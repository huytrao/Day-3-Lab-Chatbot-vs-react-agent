...
ôi trường
Truy cập terminal tại thư mục gốc của dự án và cài đặt các thư viện
cần thiết:
...
ckend (FastAPI)
Mở terminal và chạy lệnh sau để khởi động máy chủ API nội bộ (Backend sẽ mặc định ch
ạy ở cổng 8000):
...
Sau khi Backend báo khởi chạy thành công, bạn có 2
cách để mở giao diện:
...
Khởi tạo Thông tin (Init User)
Ngay khi vừa mở trang web, hệ thống sẽ hiển thị một biểu mẫu yêu cầu cung cấp thông tin để **cá nhân h
óa trải nghiệm** tư vấn của AI:
...
- **Sở thích / Yêu cầu đặc biệt:** Ví dụ như "Thích cảm giác_mạnh" hoặc "Không thích đi bộ".

👉 \*Sau khi điền đủ thông tin, hãy \_hấn nút Bắt đầu để vào giao diện

chat.\*

![Màn hình Khởi tạo thông tin](image/setup_screen.png)
...
![Giao diện Chat](image/chat_screen.png)

**Một số mẫu câu hỏi bạn
có thể thử nghiệm độ thông minh của ReAct Agent:**
...
- 🗺️ "Sắp xếp cho mình một lịch trình vui chơi trong ngày nhưng ít phải đi bộ." (Agent sẽ kết hợp Vector DB để chọn lọc trò chơi
  theo ngữ cảnh).
...
- **Tư*duy đa bước tự động (ReAct Loop)*** AI sử dụng suy luận _Thought -> Action -> Observation_ trực tiếp ở Backend để lấy dữ liệu thực tế trước khi hiển thị câu trả lời cuối cùng.
